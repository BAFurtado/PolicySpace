...
def plot(sim, text):
    ...
    full_region = geopandas.read_file('input/shapes/mun_ibge_2014_latlong_wgs1984_fixed.shp')
    ...
    for p in plots:
        ...
        if p in family_plots:
            ax = family_coords.plot(ax=ax, c=p, cmap=cmap, markersize=6, marker='.')
        else:
            ax = firms_coords.plot(ax=ax, c=p, cmap=cmap, markersize=4, marker='o')
