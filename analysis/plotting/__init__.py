...
class Plotter:
    ...
    def save_fig(self, fig, name, clear=True):
        fig.savefig(
            os.path.join(self.output_path, '{}.png'.format(name)),
            dpi=300,
            bbox_inches='tight')
    ...
    def plot_general(self):
        ...
        titles = ['Average prices\' level', 'GDP absolute value', 'GDP growth in % m-m', 'Unemployment',
                ...
                'Monthly inflation', 'Average QLI index value']
    ...
    def plot_regional_stats(self):
        ...
        # Plotting
        taxes = ['consumption', 'property', 'labor', 'firm', 'transaction', 'fpm']
        taxes_labels = ['Taxes on consumption', 'Taxes on properties', 'Taxes on labor', 'Taxes on firms',
                        'Taxes on Real Estate Transactions', 'FPM invested']
    ...
    def plot_geo(self, sim, text):
        ...
        for name, fig in figs:
            fig.savefig(os.path.join(self.output_path, 'temp_spatial_plot_{}_{}.'.format(name, text)),
                        ext='png', close=False, verbose=True, dpi=500)
        return plt
