...
class Statistics(object):
    ...
    def update_GDP_capita(self, firms, region):
        dummy_gdp = np.sum([firms[firm].revenue for firm in firms.keys() if firms[firm].region_id
                            == region.id])
        if region.pop > 0:
            dummy_gdp_capita = dummy_gdp / region.pop
        else:
    ...
    def average_qli(self, regions):
        ...
        for region in regions.keys():
            average += regions[region].index
        return average / len(regions)
